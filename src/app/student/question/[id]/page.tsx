...
export default function StudentQuestionPage() {
  ...
  async function submit() {
    ...
    const { data, error: submitError } = await supabase.rpc(
      "submit_question_page_practice",
      {
        p_question_page_id: page.id,
        p_answers: selected,
      },
    );
    ...
    setResult({
      ...
      correct: raw.correct,
      wrong: raw.wrong,
      earnedMarks: Number(raw.earned_marks),
  ...
  const percentage =
    result && result.totalMarks > 0
      ? Math.round((result.earnedMarks / result.totalMarks) * 100)
      : 0;
  const detailMap = Object.fromEntries(
    (result?.details ?? []).map((d) => [d.question_id, d]),
  );
    ...
    <div className="mx-auto max-w-4xl space-y-6 pb-12">
      <div className="flex items-start justify-between gap-4">
        <div>
          <button
            ...
            className="mb-3 inline-flex items-center gap-2 text-sm font-semibold text-muted-foreground hover:text-foreground"
          >
      ...
      {isPublicViewer && (
        <div className="rounded-2xl border border-border bg-muted/40 p-4">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            You are viewing this Question Page publicly. Questions are available to everyone; log in as a student to practise.
          </p>
        </div>
      )}
      ...
      {practiceUser && !result && (
        <div className="rounded-2xl border border-primary/20 bg-primary/5 p-4">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            You can practise this paper for free. Subscription unlocks answer-by-answer review, explanations, and discussion videos.
          </p>
        </div>
      )}
      ...
      {result && (
        <section className="overflow-hidden rounded-3xl border border-primary/20 bg-primary/5 p-6 sm:p-8">
          <div className="flex flex-col items-center text-center">
            ...
            <p className="mt-1 text-4xl font-black">{result.earnedMarks} / {result.totalMarks}</p>
            ...
            <div className="mt-4 flex flex-wrap justify-center gap-2 text-xs font-semibold">
              <span className="rounded-full bg-emerald-500/10 px-3 py-1.5 text-emerald-600">{result.correct} correct</span>
              <span className="rounded-full bg-destructive/10 px-3 py-1.5 text-destructive">{result.wrong} wrong</span>
              <span className="rounded-full bg-muted px-3 py-1.5 text-muted-foreground">{questions.length - result.answered} unanswered</span>
            </div>
            {!result.isPaid && (
              <p className="mt-4 max-w-lg text-sm text-muted-foreground">
                Your score is shown, but answer-by-answer review, correct answers, explanations and discussion videos are available to subscribed students.
              </p>
            )}
            <button
              type="button"
              onClick={resetAttempt}
              className="mt-5 inline-flex items-center gap-2 rounded-xl border border-border bg-background px-4 py-2.5 text-sm font-semibold hover:bg-muted"
            >
      ...
      {practiceUser && (
        <div className="sticky top-3 z-10 rounded-2xl border border-border bg-card/95 p-3 shadow-sm backdrop-blur sm:p-4">
          <div className="flex items-center justify-between gap-4 text-sm">
            <span className="font-bold">{result ? "Result" : "Progress"}</span>
            <span className="font-semibold text-muted-foreground">{answeredCount} / {questions.length} answered</span>
          </div>
          {!result && (
      ...
      <section className="space-y-5">
        {questions.length === 0 ? (
          <div className="rounded-2xl border border-border bg-card p-12 text-center text-sm text-muted-foreground">
            This Question Page has no questions yet.
          </div>
        ) : (
          questions.map((question, index) => {
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-xs font-bold uppercase tracking-wider text-primary">Question {question.question_number ?? index + 1}</p>
                    <h2 className="mt-1 text-lg font-bold">
                      {question.marks} mark{Number(question.marks) === 1 ? "" : "s"}
                    </h2>
                  </div>
                  {paidReview && (
                    isCorrect ? <CheckCircle2 className="h-6 w-6 shrink-0 text-emerald-600" /> : isIncorrect ? <XCircle className="h-6 w-6 shrink-0 text-destructive" /> : <CircleAlert className="h-6 w-6 shrink-0 text-muted-foreground" />
                  )}
                </div>
                ...
                {question.question_image_url ? (
                  <Image
                    ...
                    className="mt-5 max-h-[700px] w-full rounded-xl border border-border object-contain"
                  />
                ) : (
                  <div className="mt-5 flex h-28 items-center justify-center rounded-xl border border-dashed border-border text-sm text-muted-foreground">
                    Question image unavailable
                  </div>
                )}

                {page.page_type.toLowerCase() === "mcq" && practiceUser && (
                  <div className="mt-5 grid gap-3 sm:grid-cols-2">
                    {OPTIONS.map((option) => {
                      const chosenThis = chosen?.toUpperCase() === option;
                      const correctThis = paidReview && detail.correct_option?.toUpperCase() === option;
                      return (
                        <button
                          key={option}
                          type="button"
                        ...
                        >
                          <span className={`flex h-8 w-8 items-center justify-center rounded-full border text-xs ${chosenThis ? "border-primary bg-primary text-primary-foreground" : "border-border"}`}>{option}</span>
                          {chosenThis ? "Selected" : `Option ${option}`}
                        </button>
                      );
                    })}
                  </div>
                ...
                {paidReview && page.page_type.toLowerCase() === "mcq" && (
                  <div className="mt-4 rounded-xl bg-muted/50 p-4 text-sm">
                    <p className="font-semibold">{detail.selected_option ? `Your answer: ${detail.selected_option}` : "You did not answer this question."}</p>
                    {detail.correct_option && <p className="mt-1 text-muted-foreground">Correct answer: <span className="font-bold text-foreground">{detail.correct_option}</span></p>}
                    {detail.explanation && <p className="mt-3 whitespace-pre-wrap text-muted-foreground">{detail.explanation}</p>}
                    {detail.answer_image_url && <Image src={detail.answer_image_url} alt="Answer explanation" width={1000} height={750} sizes="(max-width: 768px) 100vw, 896px" className="mt-3 max-h-[600px] w-full rounded-xl border border-border object-contain" />}
                  </div>
                )}
              </article>
            );
      ...
      {practiceUser && !result && questions.length > 0 && (
        <section className="rounded-2xl border border-primary/20 bg-primary/5 p-5 sm:p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">You have answered {answeredCount} of {questions.length} questions.</p>
            </div>
            <button type="button" onClick={() => void submit()} disabled={submitting} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-primary px-5 text-sm font-bold text-primary-foreground disabled:opacity-50">
      ...
      {result?.isPaid && result.youtubeUrl && (
        <section className="rounded-2xl border border-border bg-card p-5 sm:p-6">
          <p className="font-bold">Discussion Video</p>
          <div className="mt-4 aspect-video overflow-hidden rounded-xl">
            <iframe src={getYouTubeEmbedUrl(result.youtubeUrl) ?? undefined} title="Discussion video" className="h-full w-full" allowFullScreen />
          </div>
        </section>
      )}
    </section>
