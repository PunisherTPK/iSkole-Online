...
import { BookOpen, ChevronRight, FileText, FolderTree, Search, Sparkles, X } from "lucide-react";
...
export default function QuestionBankPage() {
  ...
  const visibleSubjects = currentLevel ? subjects.filter((subject) => subject.level_id === currentLevel.id) : selectedSubject ? [] : subjects;
  ...
  function chooseSubject(subject: Subject) { setSelectedSubject(subject.id); setSelectedNode(null); setSearch(""); }
  ...
  function breadcrumbClick(index: number) { if (index === 0) { const curriculum = curriculums.find((item) => item.id === breadcrumb[0]?.id); if (curriculum) chooseCurriculum(curriculum); } else if (index === 1) { const level = levels.find((item) => item.id === breadcrumb[1]?.id); if (level) chooseLevel(level); } }
      ...
      <section className="border-b border-border/60 bg-gradient-to-b from-primary/[0.07] via-background to-background">
        <div className="mx-auto max-w-7xl px-4 pb-10 pt-12 sm:px-6 sm:pt-16 lg:px-8">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <p className="mx-auto mt-4 max-w-2xl text-base leading-7 text-muted-foreground">Browse thousands of questions by subject and topic, or search directly for the paper you need.</p>
            <div className="relative mx-auto mt-7 max-w-2xl">
              <Search className="pointer-events-none absolute left-5 top-1/2 h-5 w-5 -translate-y-1/2 text-muted-foreground" />
              <input autoComplete="off" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search questions, subjects, topics or papers..." className="h-14 w-full rounded-2xl border border-border bg-card pl-13 pr-12 text-sm font-medium shadow-lg shadow-black/[0.04] outline-none transition focus:border-primary focus:ring-4 focus:ring-primary/10" />
              {showSearch && <button type="button" aria-label="Clear search" onClick={() => setSearch("")} className="absolute right-4 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-lg text-muted-foreground hover:bg-muted hover:text-foreground"><X className="h-4 w-4" /></button>}
              {showSearch && <div className="absolute left-0 right-0 top-[calc(100%+8px)] z-40 max-h-[420px] overflow-auto rounded-2xl border border-border bg-card p-2 text-left shadow-2xl shadow-black/10">
                {searchResults.length ? searchResults.map((result) => result.kind === "page" ? <Link key={`page-${result.id}`} href={`/student/question/${result.pageId}`} onClick={() => setSearch("")} className="flex items-center gap-3 rounded-xl p-3 transition hover:bg-muted"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary"><FileText className="h-4 w-4" /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-bold">{result.title}</span><span className="block truncate text-xs text-muted-foreground">{result.subtitle}</span></span><ChevronRight className="h-4 w-4 text-muted-foreground" /></Link> : <button key={`${result.kind}-${result.id}`} type="button" onClick={() => { if (result.kind === "subject") chooseSubject(subjects.find((item) => item.id === result.id)!); else { const node = nodes.find((item) => item.id === result.id); if (node) { setSelectedSubject(node.subject_id); setSelectedNode(node.id); } } }} className="flex w-full items-center gap-3 rounded-xl p-3 text-left transition hover:bg-muted"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary"><FolderTree className="h-4 w-4" /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-bold">{result.title}</span><span className="block truncate text-xs text-muted-foreground">{result.subtitle}</span></span><ChevronRight className="h-4 w-4 text-muted-foreground" /></button>) : <div className="p-7 text-center text-sm text-muted-foreground">No matching questions, subjects or topics.</div>}
              </div>}
            </div>
          </div>
        </div>
