  ...
  async function addQuestion() {
    ...
    try {
      const nextOrder = questions.length
        ? Math.max(...questions.map((question) => question.order_index)) + 1
        : 0;
      const nextNumber = questions.length
        ? Math.max(...questions.map((question) => question.question_number ?? 0)) + 1
        : 1;
    ...
    try {
      ...
      if (answer) {
        ...
        if (answer.id) {
          ...
          setAnswers((current) => ({
            ...current,
            [question.id]: insertedAnswer as Answer,
          }));
        }
      }
    } catch (err) {
  ...
  async function handleQuestionImage(question: Question, file: File) {
    ...
    try {
      ...
      setQuestions((current) => current.map((item) =>
        item.id === question.id ? { ...item, question_image_url: url } : item,
      ));
    } catch (err) {
  ...
  async function handleAnswerImage(question: Question, file: File) {
    ...
    try {
      ...
      if (existing?.id) {
        ...
        setAnswers((current) => ({
          ...current,
          [question.id]: { ...existing, answer_image_url: url },
        }));
      } else {
        const { data, error: insertError } = await supabase
          .from("question_answers")
    ...
    return (
      <div className="mx-auto max-w-4xl space-y-6">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-primary">Preview</p>
          ...
          <button type="button" onClick={() => setPreviewMode(false)} className="inline-flex h-10 items-center gap-2 rounded-xl border border-border px-4 text-sm font-semibold hover:bg-muted">
        ...
        <section className="space-y-5 rounded-2xl border border-border bg-card p-5 sm:p-7">
          {questions.length === 0 ? (
            <div className="py-16 text-center text-sm text-muted-foreground">No questions have been added yet.</div>
          ) : questions.map((question, index) => {
            const answer = answers[question.id];
            return (
              <article key={question.id} className="rounded-2xl border border-border p-4 sm:p-6">
                <div className="flex items-center justify-between gap-3">
                  <h2 className="text-lg font-bold">Question {question.question_number ?? index + 1}</h2>
                  <span className="rounded-full bg-muted px-3 py-1 text-xs font-semibold">{question.marks} mark{Number(question.marks) === 1 ? "" : "s"}</span>
                </div>
                {question.question_image_url && <img src={question.question_image_url} alt={`Question ${question.question_number ?? index + 1}`} className="mt-4 max-h-[650px] w-full rounded-xl border border-border object-contain" />}
                {page.page_type === "mcq" && <div className="mt-5 grid gap-2 sm:grid-cols-2"><div className="rounded-xl border border-border px-4 py-3">A</div><div className="rounded-xl border border-border px-4 py-3">B</div><div className="rounded-xl border border-border px-4 py-3">C</div><div className="rounded-xl border border-border px-4 py-3">D</div></div>}
                {answer?.answer_text && <div className="mt-5 rounded-xl bg-muted/50 p-4 text-sm"><p className="font-semibold">Answer / Explanation</p><p className="mt-2 whitespace-pre-wrap text-muted-foreground">{answer.answer_text}</p></div>}
              </article>
            );
          })}
        </section>
        ...
        {videoEmbedUrl && (
          <section className="rounded-2xl border border-border bg-card p-5 sm:p-7">
            <h2 className="text-lg font-bold">Discussion Video</h2>
            <div className="mt-4 aspect-video overflow-hidden rounded-2xl bg-black">
              <iframe src={videoEmbedUrl} title="Discussion video" className="h-full w-full" allowFullScreen />
            </div>
          </section>
        )}
      </div>
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <button type="button" onClick={() => router.push("/teacher/studio")} className="mb-3 inline-flex items-center gap-2 text-sm font-semibold text-muted-foreground hover:text-foreground">
            <ArrowLeft className="h-4 w-4" /> Teacher Studio
          </button>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button type="button" onClick={() => void addQuestion()} disabled={saving} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-primary px-4 text-sm font-semibold text-primary-foreground disabled:opacity-50">
      ...
      <div className="grid gap-5 lg:grid-cols-[280px_1fr]">
        <aside className="rounded-2xl border border-border bg-card p-3">
          <div className="px-2 py-2">
            ...
            <p className="mt-1 text-xs text-muted-foreground">{questions.length} question{questions.length === 1 ? "" : "s"}</p>
          </div>
          <div className="mt-2 space-y-1">
            {questions.map((question, index) => (
              <button key={question.id} type="button" onClick={() => setSelectedId(question.id)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left ${selectedId === question.id ? "bg-primary/10 text-primary" : "hover:bg-muted/50"}`}>
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-muted text-xs font-bold">{question.question_number ?? index + 1}</span>
                <span className="min-w-0 flex-1"><span className="block text-sm font-semibold">Question {question.question_number ?? index + 1}</span><span className="block text-[11px] text-muted-foreground">{question.marks} mark{Number(question.marks) === 1 ? "" : "s"}</span></span>
              </button>
            ))}
          </div>
        ...
        <main className="min-w-0 rounded-2xl border border-border bg-card p-5">
          {!selected ? (
            <div className="flex min-h-[420px] flex-col items-center justify-center text-center">
              ...
              <h2 className="mt-4 text-lg font-bold">Start building this Question Page</h2>
              <p className="mt-2 max-w-sm text-sm text-muted-foreground">Add questions and upload question images, answers, marks and correct options here.</p>
            </div>
          ) : (
            <div className="space-y-6">
              <div className="flex items-center justify-between gap-3">
                <div><p className="text-xs font-semibold text-primary">iSkole Question {selected.question_number ?? 1}</p><h2 className="mt-1 text-lg font-bold">{page.page_type === "mcq" ? "Multiple Choice Question" : "Structured Question"}</h2></div>
                <button type="button" onClick={() => void removeQuestion(selected.id)} disabled={saving} className="rounded-xl p-2 text-destructive hover:bg-destructive/10" aria-label="Delete question"><Trash2 className="h-4 w-4" /></button>
              </div>
              ...
              <div className="rounded-2xl border border-border p-4">
                ...
                {selected.question_image_url ? <img src={selected.question_image_url} alt={`Question ${selected.question_number ?? 1}`} className="mt-4 max-h-[560px] w-full rounded-xl border border-border object-contain" /> : <div className="mt-4 flex h-36 items-center justify-center rounded-xl border border-dashed border-border text-xs text-muted-foreground">No question image uploaded</div>}
              </div>
              ...
              <div className="rounded-2xl border border-border p-4">
                ...
                <div className="mt-4 flex items-center justify-between gap-3"><div><p className="text-sm font-semibold">Answer Image</p><p className="mt-1 text-xs text-muted-foreground">Optional solution/reference image.</p></div><label className="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-border px-3 py-2 text-xs font-semibold hover:bg-muted"><Upload className="h-4 w-4" /> Upload<input type="file" accept="image/*" className="hidden" onChange={(e) => { const file = e.target.files?.[0]; if (file) void handleAnswerImage(selected, file); e.currentTarget.value = ""; }} /></label></div>
      ...
      <section className="rounded-2xl border border-border bg-card p-5">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div><p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Discussion Video</p><h2 className="mt-1 text-lg font-bold">Video explanation</h2><p className="mt-1 text-sm text-muted-foreground">Add one YouTube discussion video for this Question Page.</p></div>
          {videoEmbedUrl && <span className="rounded-full bg-emerald-500/10 px-3 py-1 text-xs font-semibold text-emerald-600">Video saved</span>}
        </div>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <button type="button" onClick={() => void saveDiscussionVideo()} disabled={videoSaving} className="inline-flex h-10 items-center gap-2 rounded-xl bg-primary px-4 text-sm font-semibold text-primary-foreground disabled:opacity-50">{videoSaving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} {videoSaving ? "Saving..." : "Save Video"}</button>
          {savedDiscussionUrl && <button type="button" onClick={() => { setDiscussionUrl(""); void saveDiscussionVideo(); }} disabled={videoSaving} className="inline-flex h-10 items-center gap-2 rounded-xl border border-border px-4 text-sm font-semibold hover:bg-muted disabled:opacity-50">Remove</button>}
        </div>
