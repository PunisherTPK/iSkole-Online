...
export default function TeacherStudioPage() {
  ...
  async function loadStudio() {
    ...
    const normalizedSubjects: Subject[] = ((subjectData ?? []) as SubjectRow[]).map((row) => {
  ...
  function toggleNode(id: string) { setExpanded((current) => { const next = new Set(current); next.has(id) ? next.delete(id) : next.add(id); return next; }); }
  function openNodeModal(parentId: string | null, editing?: Node) { setNodeName(editing?.name ?? ""); setNodeDescription(editing?.description ?? ""); setModal({ kind: "node", parentId, editing }); }
  function openQuestionModal(nodeId: string | null) { setPageTitle(""); setPageDescription(""); setPageType("mcq"); setModal({ kind: "question", nodeId }); }
  ...
  async function deleteNode(node: Node) {
    ...
    let changed = true;
    while (changed) { changed = false; for (const candidate of subjectNodes) { if (candidate.parent_id && descendants.has(candidate.parent_id) && !descendants.has(candidate.id)) { descendants.add(candidate.id); changed = true; } } }
  ...
  async function saveQuestionPage() {
    ...
    setSaving(true); setError("");
    const result = await supabase.from("question_pages").insert({ subject_id: selectedSubjectId, content_node_id: modal.nodeId, title: pageTitle.trim(), description: pageDescription.trim() || null, page_type: pageType, is_published: false });
    if (result.error) setError(result.error.message); else { setModal(null); await loadStudio(); }
    setSaving(false);
  }

  function pageMatches(page: QuestionPage) { return !term || `${page.title} ${page.description ?? ""}`.toLowerCase().includes(term); }
  function nodeMatches(node: Node) { return !term || `${node.name} ${node.description ?? ""}`.toLowerCase().includes(term); }
