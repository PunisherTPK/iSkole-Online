...
import { createTeacherContent, createTeacherQuestionPage, deleteTeacherContent } from "@/lib/teacher-actions";
...
export default async function TeacherContentPage() {
  ...
  const { data: pages } = nodeIds.length ? await supabase.from("question_pages").select("id, subject_id, content_node_id, title, page_type, is_published, created_at").in("content_node_id", nodeIds).order("created_at", { ascending: false }) : { data: [] };

  return <div className="app-page"><div className="app-page-content max-w-7xl">
    <p className="app-eyebrow">Teacher Studio</p>
    <div className="flex flex-wrap items-end justify-between gap-4"><div><h1>Content Manager</h1><p className="mt-2">Build Units, Topics, Sub Topics and Question Pages for the subjects you teach.</p></div><span className="rounded-full bg-primary/10 px-3 py-1.5 text-xs font-semibold text-primary">{profile.role === "admin" ? "Admin" : "Teacher"}</span></div>
