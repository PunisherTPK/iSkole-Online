...
export default async function TeacherContentPage() {
  ...
  const { data: rawNodes } = subjectIds.length ? await supabase.from("content_nodes").select("id, subject_id, parent_id, name, description, is_active, created_at").in("subject_id", subjectIds).order("name") : { data: [] as Node[] };
  ...
  return <div className="app-page"><div className="app-page-content max-w-7xl">
    ...
    <div className="mt-8 space-y-6">
      {subjects.map((subject) => {
        ...
        const roots = subjectNodes.filter((n) => !n.parent_id);
