...
export default async function NewQuestionPage({ searchParams }: { searchParams: Promise<{ nodeId?: string; subjectId?: string }> }) {
  ...
  if (!node) return <div className="app-page"><div className="app-page-content"><h1>Sub Topic not found</h1><Link href="/teacher/content" className="text-primary">Back to Content Manager</Link></div></div>;
  ...
  return <div className="app-page"><div className="app-page-content max-w-3xl">
    ...
    <p className="mt-2 text-muted-foreground">Create a question page under <strong>{node.name}</strong>. It will remain a draft until published.</p>
    <form action={submit} className="mt-8 space-y-5 rounded-2xl border border-border bg-card p-6 shadow-sm">
      ...
      <div><label className="text-sm font-medium" htmlFor="title">Page title</label><input id="title" name="title" required placeholder="e.g. Motion - MCQ Practice" className="mt-2 w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm"/></div>
