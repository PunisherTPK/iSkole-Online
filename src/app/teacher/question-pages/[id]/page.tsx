...
type Question = { id: string; question_number: number; question_type: string; marks: number; order_index: number; question_image_url: string | null };
...
export default async function TeacherQuestionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="app-page"><div className="app-page-content max-w-6xl">
    ...
    <section className="mt-8 rounded-2xl border border-border bg-card p-6 shadow-sm">
      ...
      <p className="mt-1 text-sm text-muted-foreground">Upload the actual question image, or paste an image URL if it already exists online. Uploaded images are stored in iSkole.</p>
      <form action={createTeacherQuestion} encType="multipart/form-data" className="mt-6 space-y-5">
        <input type="hidden" name="pageId" value={page.id}/>
        <div className="grid gap-5 md:grid-cols-2">
          <div className="rounded-xl border border-border bg-muted/20 p-4">
            <label className="text-sm font-semibold" htmlFor="questionImageFile">Upload question image</label>
            <input id="questionImageFile" name="questionImageFile" type="file" accept="image/png,image/jpeg,image/webp,image/gif" className="mt-3 block w-full text-sm"/>
            <p className="mt-2 text-xs text-muted-foreground">PNG, JPG, WEBP or GIF · maximum 8 MB</p>
          </div>
          <div className="rounded-xl border border-border bg-muted/20 p-4">
            <label className="text-sm font-semibold" htmlFor="questionImageUrl">Or use image URL</label>
            <input id="questionImageUrl" name="questionImageUrl" type="url" placeholder="https://..." className="mt-3 w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm"/>
            <p className="mt-2 text-xs text-muted-foreground">If both are provided, the uploaded image is used.</p>
          </div>
        </div>
        <div className="grid gap-3 sm:grid-cols-[1fr_130px_130px_auto]">
          <div><label className="text-sm font-medium" htmlFor="questionType">Question type</label><select id="questionType" name="questionType" defaultValue={page.page_type === "mcq" ? "mcq" : "structured"} className="mt-2 w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm"><option value="structured">Structured</option><option value="essay">Essay</option><option value="mcq">MCQ</option></select></div>
          <div><label className="text-sm font-medium" htmlFor="marks">Marks</label><input id="marks" name="marks" type="number" min="0.5" step="0.5" defaultValue="1" className="mt-2 w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm"/></div>
          <div className="hidden sm:block" aria-hidden="true" />
          <button className="self-end rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground">Add Question</button>
        </div>
      </form>
    </section>

    <section className="mt-6 space-y-3">{questions.map((question) => <article key={question.id} className="rounded-2xl border border-border bg-card p-5 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Question {question.question_number}</p><h2 className="mt-1 font-semibold">{question.question_type}</h2></div><span className="rounded-full bg-muted px-3 py-1 text-xs font-semibold">{question.marks} marks</span></div>{question.question_image_url ? <img src={question.question_image_url} alt={`Question ${question.question_number}`} className="mt-4 max-h-[700px] w-full rounded-xl border border-border object-contain"/> : <div className="mt-4 rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">No question image added yet.</div>}</article>)}{questions.length === 0 && <div className="rounded-2xl border border-dashed border-border p-10 text-center text-sm text-muted-foreground">No questions yet. Add the first question above.</div>}</section>
  </div></div>;
}
