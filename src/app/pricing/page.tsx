...
export default function PricingPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      const { data } = await supabase.from("payment_settings").select("subject_price,premium_price,currency,is_active").limit(1).maybeSingle();
      if (data) setPricing({ subjectPrice: data.subject_price, premiumPrice: data.premium_price, currency: data.currency || "LKR", active: data.is_active !== false });
      setLoading(false);
    }
    void load();
  }, []);
