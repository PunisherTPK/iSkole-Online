...
export default function MentorsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data: subjects } = subjectIds.length ? await supabase.from("subjects").select("id, name").in("id", subjectIds).eq("is_active", true) : { data: [] as { id: string; name: string }[] };
      const subjectMap = new Map((subjects ?? []).map((s) => [s.id, s]));
      const mentorData = rows.map((t) => ({ ...t, subjects: (assignments ?? []).filter((a) => a.teacher_id === t.id).map((a) => subjectMap.get(a.subject_id)).filter(Boolean) as { id: string; name: string }[] }));
      if (active) { setMentors(mentorData as Mentor[]); setLoading(false); }
    }
    void load();
    return () => { active = false; };
  }, [supabase]);
  ...
  const filtered = mentors.filter((mentor) => {
    ...
    return !q || (mentor.full_name ?? "").toLowerCase().includes(q) || mentor.subjects.some((s) => s.name.toLowerCase().includes(q));
  });
