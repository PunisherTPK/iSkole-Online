...
export default function MentorsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const subjectIds = [...new Set(assignmentRows.map((a) => a.subject_id))];
      ...
      const subjectMap = new Map(((subjects ?? []) as SubjectRow[]).map((s) => [s.id, s]));
      const mentorData: Mentor[] = rows.map((t) => ({ ...t, subjects: assignmentRows.filter((a) => a.teacher_id === t.id).map((a) => subjectMap.get(a.subject_id)).filter((s): s is SubjectRow => Boolean(s)) }));
