...
type SearchPathCurriculum = { id: string; name: string };
type SearchPathLevel = { id: string; curriculum_id: string; name: string };
type SearchPathSubject = { id: string; level_id: string; name: string };
...
export default function ContentManagerPage() {
  ...
  async function performGlobalSearch(term: string) {
    ...
    const allCurriculums = (allCurriculumsQuery.data ?? []) as SearchPathCurriculum[];
    const allLevels = (allLevelsQuery.data ?? []) as SearchPathLevel[];
    const allSubjects = (allSubjectsQuery.data ?? []) as SearchPathSubject[];

    const curriculumMap = new Map<string, SearchPathCurriculum>(allCurriculums.map((item: SearchPathCurriculum) => [item.id, item]));
    const levelMap = new Map<string, SearchPathLevel>(allLevels.map((item: SearchPathLevel) => [item.id, item]));
    const subjectMap = new Map<string, SearchPathSubject>(allSubjects.map((item: SearchPathSubject) => [item.id, item]));
