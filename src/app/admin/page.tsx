...
import { AlertCircle, ArrowRight, BookOpen, CheckCircle2, CreditCard, GraduationCap, Loader2, Users } from "lucide-react";
import { createClient } from "@/lib/supabase/client";

const modules = [
  { title: "Content Manager", description: "Manage curriculum, levels, and subjects.", href: "/admin/content-manager", icon: BookOpen },
  { title: "Teacher Studio", description: "Manage learning content and question pages.", href: "/teacher/studio", icon: GraduationCap },
...
type Counts = { students: number; teachers: number; subjects: number; subscriptions: number; pendingPayments: number };

export default function AdminPage() {
  const supabase = useMemo(() => createClient(), []);
  const [counts, setCounts] = useState<Counts>({ students: 0, teachers: 0, subjects: 0, subscriptions: 0, pendingPayments: 0 });
  ...
  useEffect(() => {
    let active = true;
    async function load() {
      const [students, teachers, subjects, subscriptions, payments] = await Promise.all([
      ...
      const failure = [students, teachers, subjects, subscriptions, payments].find((result) => result.error);
      if (!active) return;
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-2 text-sm text-muted-foreground">Manage your iSkole platform from one place.</p>
      </div>
      ...
      <section>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <AttentionCard href="/admin/students" label="Students" value={loading ? "—" : counts.students} description="Student accounts on the platform." icon={GraduationCap} />
        </div>
      </section>
      ...
      <section>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <OverviewCard label="Students" value={loading ? "—" : counts.students} icon={GraduationCap} />
          <OverviewCard label="Teachers" value={loading ? "—" : counts.teachers} icon={Users} />
          <OverviewCard label="Subjects" value={loading ? "—" : counts.subjects} icon={BookOpen} />
          <OverviewCard label="Active subscriptions" value={loading ? "—" : counts.subscriptions} icon={CreditCard} />
        </div>
      </section>
...
function AttentionCard({ href, label, value, description, icon: Icon }: { href: string; label: string; value: number | string; description: string; icon: React.ComponentType<{ className?: string }> }) {
...
function OverviewCard({ label, value, icon: Icon }: { label: string; value: number | string; icon: React.ComponentType<{ className?: string }> }) {
  return <div className="rounded-2xl border border-border bg-card p-5"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-primary/10 text-primary"><Icon className="h-4 w-4" /></div><p className="mt-5 text-2xl font-extrabold tracking-tight text-foreground">{value}</p><p className="mt-1 text-xs font-semibold text-muted-foreground">{label}</p></div>;
}
