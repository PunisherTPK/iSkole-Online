...
export async function createTeacherContent(formData: FormData) {
  ...
  if (!(await hasSubjectAccess(supabase, user.id, subjectId))) {
...
export async function deleteTeacherContent(formData: FormData) {
  ...
  if (
    !node ||
    !(await hasSubjectAccess(supabase, user.id, node.subject_id))
  ) {
    throw new Error("You cannot modify this content.");
  }
...
export async function createTeacherQuestionPage(formData: FormData) {
  ...
  if (!subjectId || !contentNodeId || !title) {
    throw new Error("Subject, content node and title are required.");
  }
  if (!(await hasSubjectAccess(supabase, user.id, subjectId))) {
    throw new Error("You are not assigned to this subject.");
  }
  if (!["mcq", "structured"].includes(pageType)) {
    throw new Error("Invalid question page type.");
  }
  const { data: node } = await supabase
    .from("content_nodes")
    .select("id, subject_id")
...
export async function createTeacherQuestion(formData: FormData) {
  ...
  const imageUrl =
    String(formData.get("questionImageUrl") || "").trim() || null;
  if (
    !pageId ||
    !["mcq", "structured", "essay"].includes(questionType) ||
  ...
  const { data: page } = await supabase
    .from("question_pages")
    .select("id, subject_id, page_type")
    .eq("id", pageId)
    .maybeSingle();
  if (
    !page ||
    !(await hasSubjectAccess(supabase, user.id, page.subject_id))
  ) {
    throw new Error("You cannot modify this question page.");
  }
  if (page.page_type === "mcq" && questionType !== "mcq") {
    throw new Error("MCQ pages can only contain MCQ questions.");
  }

  const { data: last } = await supabase
    .from("questions")
    .select("question_number, order_index")
...
async function hasSubjectAccess(
